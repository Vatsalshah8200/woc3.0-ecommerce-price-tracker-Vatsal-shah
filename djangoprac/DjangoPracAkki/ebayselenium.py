...
def ebayscript(link, price, email):
    PATH = "C:\Program Files (x86)\chromedriver.exe"
    options = Options()
    options.add_argument("--headless")
    driver = webdriver.Chrome(PATH, options=options)
    driver.get(link)
    product_name = driver.find_element_by_id("itemTitle").text
    #try:
    #search = driver.find_element_by_id('convbinPrice').text[4:]
    search = driver.find_element_by_xpath("//span[contains(@itemprop, 'price')]").get_attribute("content")
    ...
    if (float(search.replace(',',''))<=float(price)):
        ...
        filename = "D:\woc python\djangoprac\DjangoPracAkki/ebay.csv"
